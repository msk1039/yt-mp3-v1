...
def download_audio(task_id: str, url: str, celery_task=None) -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    try:
        ...
        # VPS-specific anti-bot strategies
        vps_strategies = [
            # Strategy 1: iOS client emulation (often bypasses bot detection)
            [
            ...
            # Strategy 2: Android client with different user agent
            [
            ...
            # Strategy 3: Web client with aggressive anti-detection
            [
            ...
            # Strategy 4: Minimal approach (fallback)
            [
        ...
        for strategy_num, strategy_args in enumerate(vps_strategies, 1):
            try:
                # Build command with current strategy
                cmd = base_cmd[:-1] + strategy_args + [url]  # Insert strategy before URL
                
                # Update progress with current strategy
                RedisTaskManager.update_task(
                    task_id,
                    status=TaskStatus.DOWNLOADING.value,
                    progress=20 + (strategy_num * 5),
                    message=f"Trying download strategy {strategy_num}/4..."
                )
                
                print(f"Attempting download strategy {strategy_num}: {' '.join(cmd[:5])}...")
                ...
                last_error = str(e)
                print(f"Strategy {strategy_num} exception: {e}")
                continue
